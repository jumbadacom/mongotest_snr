...
import com.jangle.mongotest_snr.mongotest_snr.api.rest.jangle.JangleRepository;
import com.jangle.mongotest_snr.mongotest_snr.api.rest.jangle.TypeCount;
import com.jangle.mongotest_snr.mongotest_snr.api.rest.tail.Tail;
import com.jangle.mongotest_snr.mongotest_snr.api.rest.tail.TailRepository;
import com.jangle.mongotest_snr.mongotest_snr.api.rest.test.TestDocument;
...
//			log.info("diff "+diff + " ms count :"+j.getRawResults().size());
			...
			/*
			for(int x=0;x<10;x++) {
				TestDocument t1 =new TestDocument("name-"+x,1L+x);
...
//			jangleList=jangleRepository.findAll(pageable).getContent();
			...
			for (int i = 0; i < 25000000; i++) {
				try {
					...
					tail.setLikeUserId(likedUsers);
					
					tail.setPassive(r.nextInt(100)<10);
					
					List<ObjectId> sharedUsers=new ArrayList<>();
					while (r.nextInt(100) <= 20) {
						sharedUsers.add(followers.get(r.nextInt(followers.size())).getId());
					}
					tail.setSharedUserId(sharedUsers);
					...
					tail.setLikeCount(tail.getLikeUserId().size());
					tail.setHideCount(tail.getHideUserId().size());
					tail.setShareCount(tail.getSharedUserId().size());
